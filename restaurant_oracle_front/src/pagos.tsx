...
const PaginaPagos: React.FC = () => {
  ...
  const mesasEnProceso = [...new Set(pedidos.filter((p) => p.status === "Entregado a la mesa").map((p) => p.table))];
  ...
  return (
    <Box p={6}>
      ...
      <VStack align="start"  mb={6}>
        ...
        <Select.Root
          collection={createListCollection({ items: mesasEnProceso.map((m) => ({ label: `Mesa ${m}`, value: m })) })}
        ...
        <Input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFacturacion({ ...facturacion, nombre: e.target.value })}
        />
        <Input
          placeholder="Número de cédula"
          value={facturacion.cedula}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFacturacion({ ...facturacion, cedula: e.target.value })}
        />
        <Input
          placeholder="Correo electrónico"
          value={facturacion.correo}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFacturacion({ ...facturacion, correo: e.target.value })}
        />
      </VStack>

      <Heading size="md" mb={2}>Resumen de Cuenta</Heading>
        ...
        <VStack align="start"  mb={4}>
          {pedidosMesa.flatMap((pedido, i) => (
            pedido.items.map((item: any, j: number) => (
              <Text key={`${i}-${j}`}>{item.quantity}x {item.name} - ${item.price * item.quantity}</Text>
            ))
          ))}
